"""Trace Qwen3-235B performance with different TP sizes and MoE backends."""
...
class TraceQwen3235B(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.base_url = DEFAULT_URL_FOR_TEST
        cls.runner = NightlyBenchmarkRunner(
            PROFILE_DIR, "Qwen3-235B Performance Traces", cls.base_url
        )
        cls.runner.setup_profile_directory()
        cls.results_manager = TraceResultsManager(PROFILE_DIR)

    def test_qwen3_235b_configs(self):
        """Test Qwen3-235B with different TP/EP/MoE configurations."""
        ...
        for tp_size, ep_size, quant_backend_pairs in CONFIGS:
            for (
                ...
                config_name = f"qwen3-235b {variant}"
                ...
                if not primary_success and fallback_backend is not None:
                    ...
                    fallback_config_name = f"qwen3-235b {fallback_variant}"
